...
export default function RevenueMoney({ currentDate, setCurrentDate }: any) {
  const [revenueMoney, setRevenueMoney] = useState(0);
  const [isPickerVisible, setIsPickerVisible] = useState(false);
  const [selectedCategoryID, setSelectedCategoryID] = useState(
    DEFAULT_REVENUECATEGORIES[0].id
  );
  const { revenueCategories = [] } = useContext(CategoryContext) || {};
    ...
    <View>
      ...
      <View className="flex-row items-center gap-x-[11px] px-[13px] py-[7px] border-b border-borderDark">
        ...
        <TextInput
          ...
          value={String(revenueMoney)}
          onChangeText={(text) => {
            const numericValue = parseInt(text) || 0;
            setRevenueMoney(numericValue);
          }}
      ...
      <View className="items-center pt-5">
        <EnterBtn title="Nhập khoản thu" onPress={handleAdd} />
      </View>
    </View>
  );
}
